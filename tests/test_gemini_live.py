...
pytestmark = pytest.mark.skipif(not (RUN_LIVE and API_SET), reason='Set RUN_GEMINI_INTEGRATION=1 and GOOGLE_API_KEY to run')
...
def test_gemini_live_corrections_and_proper_names(tmp_path: Path):
    ...
    process_document(
        str(input_doc), str(out_doc), str(log_json), corrector,
        chunk_words=0, overlap_words=0, preserve_format=True, enable_docx_log=False
    )
