    ...
    def __init__(self, model_name: str | None = None, base_prompt_text: str | None = None) -> None:
        ...
        if model_name is None:
            try:
                ...
                model_name = settings.gemini_model if settings and settings.gemini_model else "gemini-2.5-flash"
            except Exception:
    ...
    def correct_tokens(self, tokens: List[Token]) -> List[CorrectionSpec]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"Caught {error_type}: is_server_error={is_server_error}, is_rate_limit={is_rate_limit}, attempt={attempt}/{max_retries}")
                ...
                if (is_server_error or is_rate_limit) and attempt < max_retries - 1:
                    ...
                    if is_rate_limit and retry_delay:
                        delay = retry_delay
                        logger.warning(f"⚠️  Rate limit exceeded (429), retrying in {delay}s... (attempt {attempt + 1}/{max_retries})")
                    else:
                        # Exponential backoff: 2s, 4s, 8s
                        delay = base_delay * (2 ** attempt)
                        logger.warning(f"⚠️  Model overloaded (503), retrying in {delay}s... (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
                    ...
                    logger.warning(f"⚠️  {self.model_name} failed after {max_retries} retries ({error_type}), trying Azure OpenAI GPT-5")
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self._client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": "You are a text analysis assistant that returns JSON."},
                ...
                if is_content_filter:
                    ...
                    try:
                        ...
                        if settings.azure_openai_fallback_deployment_name:
                            ...
                            fallback_api_version = settings.azure_openai_fallback_api_version or "2025-01-01-preview"

                            logger.info(f"🤖 Using Azure OpenAI fallback model: {fallback_deployment}")
