...
try:
    # Crear usuario
    response = requests.post('http://localhost:8001/auth/register', data=json.dumps(user_data), headers=headers)
    ...
    if response.status_code == 200:
        ...
        login_response = requests.post('http://localhost:8001/auth/login', data=json.dumps(user_data), headers=headers)
